    ...
    public class CustomerController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                if (TenantId > 0)
                {
                    var dto = await _customerRepository.GetAllByTenantCompanyIdAsync(TenantId, CompanyId);
                    return View(dto);
                }
                else
                {
                    var dto = await _customerRepository.GetAllAsync();
                    return View(dto);
                }
            }
            catch (Exception ex)
        ...
        public async Task<IActionResult> Create()
        {
            try
            {
                ...
                if (TenantId > 0)
                {
                    ...
                    dto.Code = await _tenantPrefixService.InitializeGenerateCode(TenantId,CompanyId, "", PrefixType.Customer,true);
                    return View(dto);
                }
                else
                {
        ...
        public async Task<IActionResult> Create(CustomerDto dto)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    dto.Code = await _tenantPrefixService.InitializeGenerateCode(TenantId,CompanyId, dto.Code, PrefixType.Customer, false);
                    var id = await _customerRepository.AddAsync(dto);
                    if (id > 0)
                    {
        ...
        public async Task<IActionResult> Edit(CustomerDto dto)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    dto.TenantId = TenantId;
                    dto.CompanyId = CompanyId;
                    var id = await _customerRepository.UpdateAsync(dto);
                    if (id > 0)
                    {
                        return RedirectToAction(nameof(Index));
                    }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                var module = await _customerRepository.GetByIdAsync(id);
                if (module == null)
                    return NotFound();
        ...
        private async Task<CustomerDto> BuildObjectAsync()
        {
        ...
        //============== Openig Modal ==============
        #region Customer_Opening_Modal
        [HttpGet]
        public async Task<IActionResult> CreatePartial(string CustomerComingFrom = null)
        {
            try
            {
                CustomerDto dto = await BuildObjectAsync();
                dto.CustomerCommingFrom = CustomerComingFrom == "POS" ? "POS" : "simple";
                return PartialView("_CreateCustomerPartial", dto);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> CreatePartialPost(CustomerDto model)
        {
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                ...
                var newId = await _customerRepository.AddAsync(model);
