...
namespace WebdeskEA.Areas.Admin.Controllers
{
    ...
    public class OSController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                IEnumerable<OSDto> dto;
                if (TenantId > 0)
                {
                    dto = await _OSRepository.GetAllByTenantCompanyFinancialYearIdAsync(TenantId, CompanyId,FinancialYearId);
                }
                else
                {
                    dto = null;
        ...
        public async Task<IActionResult> Create()
        {
            ...
            {
                OSDto dto = new OSDto();
                dto.ProductDtos = await _productRepository.GetAllByTenantCompanyFinancialYearsIdAsync(TenantId, CompanyId, FinancialYearId);
                dto.OSCode = await _tenantPrefixService.InitializeGenerateCode(TenantId,CompanyId, "", PrefixType.OS, true);
                return View(dto);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Create(OSDto dto)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var id = await _OSRepository.AddTransactionAsync(dto);
                    if (id > 0)
                    {
        ...
        public async Task<IActionResult> Edit(int id)
        {
            try
            {
                var dto = await _OSRepository.GetByIdAsync(id) ?? new OSDto();
                dto.ProductDtos = await _productRepository.GetAllByTenantCompanyFinancialYearsIdAsync(TenantId, CompanyId, FinancialYearId);
                dto.OSDtlDtos = await _OSDtlRepository.GetAllByOSIdAsync(id);
                if (dto == null)
                    return NotFound();
        ...
        // OSST: Modules/Edit/{id}
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(OSDto dto)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    dto.CompanyId = CompanyId;
                    dto.FinancialYearId = FinancialYearId;
                    var id = await _OSRepository.UpdateTransactionAsync(dto);
                    if (id > 0)
                    {
                        return RedirectToAction(nameof(Index));
                    }
                ...
                dto.ProductDtos = await _productRepository.GetAllByTenantCompanyFinancialYearsIdAsync(TenantId, CompanyId, FinancialYearId);
                dto.OSDtlDtos = await _OSDtlRepository.GetAllByOSIdAsync(dto.Id);
                return View(dto);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                var module = await _OSRepository.GetByIdAsync(id);
                if (module == null)
                    return NotFound();
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            try
            {
                ...
                var result = await _OSRepository.DeleteAsync(id);
                if (result > 0)
                {
                    return RedirectToAction(nameof(Index));
                }
        ...
        /// <summary>
        /// this is being used in PI  on  OS Drpdown to fetch the SO list in SI
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        ...
        public async Task<IActionResult> CheckPIExists(int poId)
        {
            try
            {
                var dto = await _OSRepository.GetByIdAsync(poId) ?? new OSDto();
                if (dto == null)
                    return Json(new { success = false, message = "Data not found." }); // Return failure response for AJAX
        ...
        public async Task<IActionResult> GotoEditPI(int poId)
        {
            ...
            //try
            //{
            //    var dto = await _PIRepository.GetByOSIdAsync(poId) ?? new PIDto();
            //    if (dto == null)
            //        return Json(new { success = false, message = "Data not found." }); // Return failure response for AJAX
        ...
        public async Task<IActionResult> GeneratePI([FromBody] GeneratePIRequest request)
        {
            ...
            //    var siResult = await _PIRepository.GetByOSIdAsync(request.PoId) ?? new PIDto();
            //    if (siResult.Id > 0)
            //        return Json(new { success = false, message = "Invoice Alredy Generated." });

            //    var dto = await _OSRepository.GetByIdAsync(request.PoId) ?? new OSDto();
            //    dto.TenantId = TenantId;
            //    dto.CompanyId = CompanyId;
            //    dto.OSDtlDtos = await _OSDtlRepository.GetAllByOSIdAsync(dto.Id);
            //    dto.OSCode = await _tenantPrefixService.InitializeGenerateCode(TenantId, "", PrefixType.PI, true);
            //    var result = await _OSRepository.GeneratePIByOS(dto);
            //    if (!result)
        ...
        public async Task<IActionResult> DeletePI([FromBody] GeneratePIRequest request)
        {
            ...
            //    var result = await _OSRepository.DeletePIByOS(request.PoId);
            //    if (!result)
