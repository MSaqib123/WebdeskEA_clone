...
new Promise((resolve, reject) => {
    ...
    // Submit
    document.querySelector('#js-builder-preview').addEventListener('click',async function () {
        ...
        // 1) Gather chosen values
        const skin = document.querySelector('[name="layoutSkinsRadio"]:checked').value;  // 'default' or 'dark'
        const sidebar = document.querySelector('[name="sidebarNavOptions"]:checked');
        const sidebarVal = sidebar ? sidebar.value : null;                                // 'pills' or 'tabs'
        const layoutVal = document.querySelector('[name="layout"]:checked').value;        // 'default', 'navbar-dark', ...

        // 2) Immediately apply on the client
        ...
        // 3) Optionally call your server endpoint to save each setting
        // Suppose you have an endpoint: /GlobalSettings/SetUserSetting
        // that expects a JSON body with { key, value } and sets the setting for the logged-in user:
        try {
            // Save Skin
            await fetch('/Common/GlobalSettings/SetUserSetting', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ key: 'theme', value: skin })
            });
            ...
            await fetch('/Common/GlobalSettings/SetUserSetting', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ key: 'layout', value: layoutVal })
            });
            ...
            await fetch('/Common/GlobalSettings/SetUserSetting', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ key: 'sidebarNavOptions', value: sidebarVal })
            });
            ...
            await fetch('/Common/GlobalSettings/SetUserSetting', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ key: 'builderFluidSwitch', value: fluid.toString() })
            });

        } catch (err) {
            console.error('Failed to save setting(s):', err);
        }
